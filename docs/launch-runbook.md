...
### 2026-09-07 — the study, and the sitemap that was quietly wrong

**Published the first measurement of documentation accessibility in Confluence.**
582 pages, seven public sites, measured by our own engine against WCAG 2.2. 57% carried
no level A or AA failure, no site was clean, and six checks accounted for 87% of the
2,508 issues found. `site/src/study-confluence-accessibility.html`; harness and method in
`research/`.

Two things worth remembering about how it was produced:

- The first run reported 90% clean. It took the first pages of each space, which are
  landing pages with no images and no tables. Random sampling from the full page pool
  moved it to 57%. The plumbing was never the hard part; the sampling was.
- Our own checker flagged the study page's summary table for a missing header row before
  it shipped. An accessibility study that fails its own check would have been the single
  most expensive mistake available today.

**The sitemap was hand-written and had silently missed five pages**, including the study
— exactly the search visibility those pages exist for. It is now generated by
`tools/build-site.mjs` and cannot go stale again. 37 pages, 37 urls, verified live.

**Community outreach held.** The Atlassian Community forums migrate to a new platform on
22 September; the answer editor rejects submissions today. Holding the four drafted
replies until after the migration rather than posting into a system being replaced.

The study post leads the LinkedIn queue on 8 September, replacing a generic tips post. A
page's first post should be the thing only it could have written.
